    ...
    <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#131B2F] border border-white/5 p-6 rounded-2xl shadow-lg">
          <p className="text-sm font-medium text-slate-400 mb-2">Gross Volume (30d)</p>
          <h3 className="text-4xl font-extrabold text-white">$142,500</h3>
        </div>
        <div className="bg-[#131B2F] border border-emerald-500/30 p-6 rounded-2xl shadow-[0_0_30px_rgba(16,185,129,0.1)] relative overflow-hidden">
          ...
          <h3 className="text-4xl font-extrabold text-white relative z-10">$28,500</h3>
        </div>
        <div className="bg-[#131B2F] border border-white/5 p-6 rounded-2xl shadow-lg">
          <p className="text-sm font-medium text-slate-400 mb-2">Pending Payouts</p>
            ...
            <tbody className="divide-y divide-white/5">
              {payouts.map((payout) => (
                <tr key={payout.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="p-6 text-sm text-slate-400">{payout.method}</td>
                  <td className="p-6 text-right font-bold text-white">{payout.amount}</td>
                  <td className="p-6 text-center">
                    <span className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${payout.status === 'Pending' ? 'bg-amber-500/10 text-amber-400' : 'bg-slate-500/10 text-slate-400'}`}>
                      {payout.status}
                    </span>
                  </td>
                  <td className="p-6 text-right">
                    {payout.status === 'Pending' ? (
                      <button className="px-4 py-2 bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600 hover:text-white text-xs font-bold rounded-lg transition-colors border border-emerald-500/30">Process</button>
                    ) : (
